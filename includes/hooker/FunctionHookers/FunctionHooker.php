...
abstract class FunctionHooker extends Hooker {
    ...
    protected function hookSpecific($data, $handler) {
        ...
        foreach($data['tags'] as $hook) { /* Iterate over WP tags */
            ...
            if((array_key_exists('cron', $hook) && !empty($hook['cron'])) && static::class === 'ActionHooker') /* This is a cron task, register it into CronManager */
                CronManager::registerEvent($tag, (string) $hook['cron']);
